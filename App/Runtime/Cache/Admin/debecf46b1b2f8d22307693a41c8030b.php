...
<body>
    <form action="/index.php?s=/admin/agreement/save.html" id="registerform">
        <input type="hidden" name="id" value="<?php echo ($data["id"]); ?>">
    <table class="table table-bordered table-hover definewidth m10">
        <tr>
            <td width="10%" class="tableleft">协议名称<span class="red">*</span></td>
            <td><input type="text" value="<?php echo ($data["name"]); ?>" name="name" class="inputxt" datatype="*" nullmsg="请输入协议名称！" errormsg="请输入协议名称！" style="width:500px;" /></td>
        </tr>
        <tr>
            <td class="tableleft">协议</td>
	...
	$("#registerform").Validform({
        ...
        callback:function(data){
            if(data.status == 'y'){
                $.Hidemsg();
                alert(data.msg);
                    <?php if(!empty($data)): ?>window.location.href="/index.php?s=/admin/agreement/list.html";
                    <?php else: ?>
                        window.location.href="/index.php?s=/admin/agreement/edit/"+data.id+".html";<?php endif; ?>
            }else{
                alert(data.msg);
            }
        return false;
