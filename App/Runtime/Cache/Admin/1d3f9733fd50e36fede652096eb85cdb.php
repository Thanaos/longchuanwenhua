...
<body>
    <form action="/index.php/admin/service/branchSave.html" id="registerform">
        <input type="hidden" name="id" value="<?php echo ($data["id"]); ?>">
    <table class="table table-bordered table-hover definewidth m10">
        <tr>
            <td width="10%" class="tableleft">服务点名<span class="red">*</span></td>
            <td><input type="text" value="<?php echo ($data["branch_name"]); ?>" name="branch_name" class="inputxt" datatype="*" nullmsg="请输入服务点名！" errormsg="请输入服务点名！" style="width:500px;" /></td>
        </tr>
        <tr>
            <td width="10%" class="tableleft">服务点用户手机号<span class="red">*</span></td>
            <td><input type="text" value="<?php echo ($data["mobile"]); ?>" name="mobile" class="inputxt"  style="width:500px;" datatype="*" nullmsg="请输入手机号！" errormsg="请输入手机号！" /><span style="color:#ccc">（填写相应员工的手机号。便于系统通知地点）</span></td>
        </tr>
        <tr>
            <td width="10%" class="tableleft">经度<span class="red">*</span></td>
            <td><input type="text" value="<?php echo ($data["longitude"]); ?>" name="longitude" class="inputxt" datatype="*" nullmsg="请输入经度！" errormsg="请输入经度！" style="width:500px;" /><span style="color:#ccc">（请使用百度地图选好点后，填写。须精确小数点后六位）</span></td>
        </tr>
        <tr>
            <td width="10%" class="tableleft">纬度<span class="red">*</span></td>
            <td><input type="text" value="<?php echo ($data["latitude"]); ?>" name="latitude" class="inputxt" datatype="*" nullmsg="请输入纬度！" errormsg="请输入纬度！" style="width:500px;" /></td>
        </tr>        
        <tr>
	...
	$("#registerform").Validform({
        ...
        callback:function(data){
            if(data.status == 'y'){
                $.Hidemsg();
                alert(data.msg);
                        window.location.href="/index.php/admin/service/branch.html";
            }else{
                alert(data.msg);
            }
        return false;
