...
<body>
    <form action="/index.php?s=/admin/flash/save.html" id="registerform">
        <input type="hidden" name="id" value="<?php echo ($data["id"]); ?>">
    <table class="table table-bordered table-hover definewidth m10">
        <tr>
            <td width="10%" class="tableleft">轮播名称<span class="red">*</span></td>
            <td><input type="text" value="<?php echo ($data["name"]); ?>" name="name" class="inputxt" datatype="*" nullmsg="请输入轮播名称！" errormsg="请输入轮播名称！" style="width:500px;" /></td>
        </tr>
        <tr>
            <td width="10%" class="tableleft">跳转网址<span class="red">*</span></td>
            <td><input type="text" value="<?php echo ($data["url"]); ?>" name="url" class="inputxt" datatype="url" nullmsg="请输入跳转网址！" errormsg="请输入正确的网址！" style="width:500px;" /> <span style="color:#ccc">请输入不带http开头的网址</span></td>
        </tr>
        <tr>
	...
	$("#registerform").Validform({
        ...
        callback:function(data){
            if(data.status == 'y'){
                $.Hidemsg();
                alert(data.msg);
                    <?php if(!empty($data)): ?>window.location.href="/index.php?s=/admin/flash/list.html";
                    <?php else: ?>
                        window.location.href="/index.php?s=/admin/flash/edit/"+data.id+".html";<?php endif; ?>
            }else{
                alert(data.msg);
            }
        return false;
